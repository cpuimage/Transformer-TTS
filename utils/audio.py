...
def trim_silence(wav, hparams):
    trimed = librosa.effects.trim(wav, top_db=hparams.trim_top_db, frame_length=hparams.trim_fft_size,
                                  hop_length=hparams.trim_hop_size)[0]

    return trimed


def linearspectrogram(wav, hparams, use_preemphasis=False):
    if use_preemphasis:
...
def melspectrogram(wav, hparams, use_preemphasis=False):
    if use_preemphasis:
        D = _stft(preemphasis(wav, hparams.preemphasis, hparams.preemphasize), hparams)
    else:
        D = _stft(wav, hparams)
...
def _griffin_lim_tensorflow(S, hparams):
    ...
    with tf.variable_scope('griffinlim'):
        ...
        y = tf.contrib.signal.inverse_stft(S_complex, hparams.win_size, hparams.hop_size, hparams.n_fft)
        for i in range(hparams.griffin_lim_iters):
            est = tf.contrib.signal.stft(y, hparams.win_size, hparams.hop_size, hparams.n_fft)
            angles = est / tf.cast(tf.maximum(1e-8, tf.abs(est)), tf.complex64)
            y = tf.contrib.signal.inverse_stft(S_complex * angles, hparams.win_size, hparams.hop_size,
                                               hparams.n_fft)
...
def _istft(y, hparams):
    return librosa.istft(y, hop_length=hparams.hop_size, win_length=hparams.win_size)
